...
Moreover most of not all of the MQ services that we are familiar with are based on TCP/IP
which due to its streaming nature and also the way it which it handles connection
setup and teardown as well as failed connections tends to add high latency to
...
sacrificing latency and scalability.
